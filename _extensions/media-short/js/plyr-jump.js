// /_extensions/media-short/plyr-jump.js
...
(function () {
  ...
  // Ensure that audio is actually on for user-initiated jumps
  function ensureSoundOn(player) {
    if (!player) return;

    // If player is muted, unmute it
    if (player.muted) {
      player.muted = false;
    }

    // If volume is zero or invalid, restore to a safe default
  ...
  function seekAndPlay(player, seconds, opts) {
    ...
    try {
      // Deep-link senaryosu: sadece seek, play etme
      if (!autoPlay) {
        try {
          player.currentTime = seconds;
        } catch (e) {
          console.warn("Error setting currentTime (autoPlay=false):", e);
        }
        return;
      }

      // Zaten oynuyorsa: sadece seek et, ses durumuna dokunma
      if (player.playing) {
        try {
          player.currentTime = seconds;
        } catch (e) {
          console.warn("Error setting currentTime (already playing):", e);
        }
        // Autoplay ile sessiz oynayan video için:
        // Kullanıcı click'i içinden geldiğimiz için
        // burada sesi açmak autoplay politikasına takılmaz.
        if (player.muted || player.volume === 0) {
          ensureSoundOn(player);
        }
        return;
      }

      // --- ÖZEL DURUM: YouTube ve henüz hiç başlamamış (ilk play) ---
      // Bu durumda önce play, sonra seek yapıyoruz ki autoplay politikası sese dokunmasın.
      if (isYouTube && player.currentTime === 0) {
        // Sesin açık olduğundan emin ol (kullanıcı tıklaması içinde)
        ensureSoundOn(player);

        const target = seconds;

        // Sadece bir kez çalışacak "playing" dinleyicisi
        const applyOnce = () => {
          try {
            player.currentTime = target;
          } catch (e) {
            console.warn("Error setting currentTime in playing handler:", e);
          }
          if (typeof player.off === "function") {
            player.off("playing", applyOnce);
          }
        };

        if (typeof player.on === "function") {
          player.on("playing", applyOnce);
        }

        // Plyr'ın kendi play butonuna tıklamayı simüle et
        let playButton = null;
          ...
          try {
            ...
            if (maybePromise && typeof maybePromise.then === "function") {
              maybePromise.catch(function (err) {
                console.warn("player.play() rejected (YouTube first play):", err);
              });
            }
          } catch (e) {
            console.warn("Fallback player.play() error (YouTube first play):", e);
          }
        }
      ...
      // --- Diğer tüm durumlar (local video/audio, YouTube ama daha önce oynatılmış) ---
      try {
        player.currentTime = seconds;
      } catch (e) {
        console.warn("Error setting currentTime (generic case):", e);
      }

      // Burada da sesin açık olduğundan emin olalım
      ensureSoundOn(player);

      // Paused ise Plyr’ın kendi play butonuna tıklamayı simüle et
      let playButton = null;
      ...
      if (playButton && typeof playButton.click === "function") {
        ...
        try {
          ...
          if (maybePromise && typeof maybePromise.then === "function") {
            maybePromise.catch(function (err) {
              console.warn("player.play() rejected (generic case):", err);
            });
          }
        } catch (e) {
          console.warn("Fallback player.play() error (generic case):", e);
        }
      }
    } catch (e) {
